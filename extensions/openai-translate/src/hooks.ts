...
import { LanguageCodeSet, Preferences } from "./types";
import { AUTO_DETECT } from "./openai-translate";
...
export const useSourceLanguage = () => {
  const [sourceLanguage, setSourceLanguage] = useCachedState<LanguageCode>("sourceLanguage", AUTO_DETECT as LanguageCode);
...
export const useTargetLanguages = () => {
  ...
  const [targetLanguages, setTargetLanguages] = useCachedState<LanguageCode[]>(
    "targetLanguages",
    [preferences.lang1, preferences.lang2].filter((lang) => lang !== AUTO_DETECT) as LanguageCode[]
  );
